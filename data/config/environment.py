...
class EnvironmentConfig:
    ...
    def _build_config(self):
        ...
        self.config = {
            ...
            'sendgrid': {
                ...
                'enabled': os.getenv('SENDGRID_ENABLED', 'true').lower() == 'true'
            },
    ...
    def _validate_config(self):
        ...
        # Validation Twilio SMS (si activé)
        if self.config['twilio']['enabled']:
            if not self.config['twilio']['account_sid']:
                errors.append("TWILIO_ACCOUNT_SID requis quand Twilio est activé")
            if not self.config['twilio']['auth_token']:
                errors.append("TWILIO_AUTH_TOKEN requis quand Twilio est activé")
            if not self.config['twilio']['phone_number']:
                errors.append("TWILIO_PHONE_NUMBER requis quand Twilio est activé")

        # Validation SendGrid Email (si activé)
        if self.config['sendgrid']['enabled']:
            if not self.config['sendgrid']['api_key']:
                errors.append("SENDGRID_API_KEY requis quand SendGrid est activé")
            if not self.config['sendgrid']['from_email']:
                errors.append("SENDGRID_FROM_EMAIL requis quand SendGrid est activé")
    ...
    def get_twilio_config(self) -> Dict[str, Any]:
        """Retourne la configuration Twilio complète"""
        return self.config['twilio']

    def get_sendgrid_config(self) -> Dict[str, Any]:
        """Retourne la configuration SendGrid complète"""
        return self.config['sendgrid']

    def is_sms_enabled(self) -> bool:
        """Vérifie si les SMS Twilio sont activés"""
        return self.config['twilio']['enabled']

    def is_email_enabled(self) -> bool:
        """Vérifie si les emails SendGrid sont activés"""
        return self.config['sendgrid']['enabled']
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        if summary['twilio']['auth_token']:
            summary['twilio']['auth_token'] = '***masked***'
        if summary['sendgrid']['api_key']:
            summary['sendgrid']['api_key'] = '***masked***'
