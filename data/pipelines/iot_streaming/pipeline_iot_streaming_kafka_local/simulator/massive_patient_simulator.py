...
# Configuration base de données PostgreSQL - CRÉDENTIELS ACTUALISÉS
DB_CONFIG = {
...
TWILIO_CONFIG = {
    ...
    'to_number': os.getenv('TWILIO_TO_NUMBER', '+1234567890')  # Numéro cible pour SMS alertes
}
...
EMAIL_CONFIG = {
    ...
    'password': os.getenv('SMTP_PASSWORD', 'your_sendgrid_api_key'),
    'from_email': os.getenv('FROM_EMAIL', 'support@kidjamo.app'),
    'to_email': os.getenv('TO_EMAIL', 'alerts@kidjamo.app')  # Email cible pour alertes
}
...
class MeasurementRecord:
    ...
    def to_db_record(self) -> Tuple:
        """Convertit en tuple pour insertion batch PostgreSQL - Compatible avec schéma existant"""
        return (
            self.patient_id, self.device_id,
...
class AlertRecord:
    ...
    title: str
    message: str
    created_at: datetime  # Déplacé avant les champs avec valeurs par défaut
    ...
    def to_db_record(self) -> Tuple:
        """Convertit en tuple pour insertion PostgreSQL - Compatible avec schéma existant"""
        return (
            self.patient_id, self.alert_type, self.severity,
            self.title, self.message,
            json.dumps(self.vitals_snapshot),
            self.trigger_conditions,  # Pass as Python list, PostgreSQL will convert to array
            self.suggested_actions,   # Pass as Python list, PostgreSQL will convert to array
...
Actions: {', '.join(alert.suggested_actions[:2])}"""
    ...
    def send_notification_sync(self, alert: AlertRecord, patient: PatientProfile):
        """Version synchrone pour envoi notifications - appelée depuis thread séparé"""
        try:
            # Préparer contenu notification
            sms_content = self._format_sms_alert(alert, patient)
            email_content = self._format_email_alert(alert, patient)

            # Envoi SMS si critique ou alerte (version synchrone)
            if alert.severity in ['critical', 'alert'] and not self.circuit_breaker['sms_disabled']:
                self._send_sms_sync(sms_content)

            # Envoi Email pour toutes les alertes (version synchrone)
            if not self.circuit_breaker['email_disabled']:
                self._send_email_sync(alert.title, email_content)

            logger.info(f"📤 Notifications envoyées (sync) pour alerte {alert.alert_type} - Patient {patient.first_name} {patient.last_name}")

        except Exception as e:
            logger.error(f"❌ Erreur envoi notifications (sync): {e}")

    def _send_sms_sync(self, content: str):
        """Version synchrone d'envoi SMS"""
        if not self.twilio_client:
            logger.warning("⚠️  Client Twilio non disponible")
            return
        ...
        try:
            ...
            self.circuit_breaker['sms_failures'] = 0
            self.circuit_breaker['last_sms_success'] = datetime.now()
            logger.info(f"📱 SMS envoyé avec succès (sync): {message.sid}")

        except Exception as e:
            self.circuit_breaker['sms_failures'] += 1
            logger.error(f"❌ Erreur envoi SMS (sync): {e}")
    ...
    def _send_email_sync(self, subject: str, html_content: str):
        """Version synchrone d'envoi Email"""
        if not EMAIL_CONFIG['username']:
            logger.warning("⚠️  Configuration SMTP manquante")
            return
        ...
        try:
            ...
            self.circuit_breaker['email_failures'] = 0
            self.circuit_breaker['last_email_success'] = datetime.now()
            logger.info(f"📧 Email envoyé avec succès (sync)")

        except Exception as e:
            self.circuit_breaker['email_failures'] += 1
            logger.error(f"❌ Erreur envoi Email (sync): {e}")
