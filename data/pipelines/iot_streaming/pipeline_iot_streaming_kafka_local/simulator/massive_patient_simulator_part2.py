...
class DatabaseManager:
    ...
    def flush_patients(self):
        ...
        try:
            ...
            # Create a system user if it doesn't exist to handle audit logs
            system_user_query = """
                INSERT INTO users (
                    user_id, role, first_name, last_name, gender, 
                    email, password_hash, phone, city, country, 
                    is_active, created_at, updated_at
                ) VALUES (
                    '00000000-0000-0000-0000-000000000000', 'admin', 'System', 'User', 'M',
                    'system@kidjamo.app', 'system_hash', '+000000000000', 'System', 'System',
                    true, NOW(), NOW()
                ) ON CONFLICT (user_id) DO NOTHING
            """
            cursor.execute(system_user_query)

            # Set the current user for audit purposes
            cursor.execute("SET LOCAL app.current_user_id = '00000000-0000-0000-0000-000000000000'")
    ...
    def flush_measurements(self):
        """Flush buffer mesures vers PostgreSQL avec résolution de conflits"""
        ...
        try:
            ...
            # Requête avec ON CONFLICT pour gérer les doublons
            query = """
