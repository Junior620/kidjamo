...
def erf_stable(x: np.ndarray) -> np.ndarray:
    ...
    # approximation polynomiale de erf (Abramowitz & Stegun-like)
    # erf(x) ≈ sign(x) * sqrt(1 - exp(-x^2*(4/pi + ax^2)/(1+a x^2)))
    a = 0.147  # constante d'approximation
    sign = np.sign(x)
    x2 = x * x
    inner = -(x2 * (4.0 / math.pi + a * x2)) / (1.0 + a * x2)
    return sign * np.sqrt(1.0 - np.exp(inner))
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Générateur de dataset synthétique amélioré (100k lignes) à partir d'un CSV.")
    parser.add_argument("--src", required=True, help="Chemin du CSV source (ex: data.csv)")
